...
public class HomeFragment extends Fragment implements HomeContract.View,
    ...
    private void setupClickListeners() {
        binding.mealOfDayCard.setOnClickListener(v -> {
            if (mealOfTheDay != null) presenter.onMealOfTheDayClicked(mealOfTheDay);
        });
    ...
    public void showMealOfTheDay(MealItemModel meal) {
        ...
        Glide.with(this).load(meal.getCountryFlagUrl()).placeholder(R.drawable.flag).into(binding.mealOfTheDaymealCountryFlag);
        Glide.with(this).load(meal.getImageUrl()).placeholder(R.drawable.plan1).into(binding.mealOfDayImage);
    }

    @Override
    public void showCategories(List<Category> categories) {
        if (isAdded()) categoriesAdapter.setCategories(categories);
    }

    @Override
    public void showPopularMeals(List<MealItemModel> meals) {
        if (isAdded()) popularAdapter.setMeals(meals);
    }

    @Override
    public void showCuisines(List<Area> areas) {
        if (isAdded()) cuisinesAdapter.setCuisines(areas);
    }
    ...
    public void onCuisineClicked(Area cuisine) {
        HomeFragmentDirections.ActionHomeFragmentToSearchFragment action =
                HomeFragmentDirections.actionHomeFragmentToSearchFragment();
        action.setAreaName(cuisine.getName());
        NavHostFragment.findNavController(this).navigate(action);
    }
